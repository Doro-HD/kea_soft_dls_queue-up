...
RUN pnpm deploy --filter=qu_synchronizer_api --prod /prod/qu_synchronizer_api
...
# synchronizer
FROM base AS qu_synchronizer_api
WORKDIR /app
...
COPY --from=builder --chown=hono:nodejs /prod/qu_synchronizer_api/.env ./.env
COPY --from=builder --chown=hono:nodejs /prod/qu_synchronizer_api/node_modules ./node_modules
COPY --from=builder --chown=hono:nodejs /prod/qu_synchronizer_api/dist ./dist
COPY --from=builder --chown=hono:nodejs /prod/qu_synchronizer_api/package.json ./package.json
